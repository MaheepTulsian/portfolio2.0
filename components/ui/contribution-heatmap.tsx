...
export function HeatmapCalendar({
  ...
  const LegendUI = renderLegend ? (
    ...
    <div className={cn("min-w-35 flex-shrink-0", legendCfg.className)}>
      {showText ? (
  ...
  return (
    <TooltipProvider delayDuration={80}>
      <div className={cn("flex gap-4 w-full", placement === "bottom" && "flex-col", className)}>
        {/* Labeled calendar area */}
        <div className={cn("min-w-0 flex-1 flex flex-col", axisCfg.className)}>
          {/* Month labels row */}
          {showAxis && showMonths ? (
